...
router.get("/", (req, res) => {
  Course.find({})
    ...
    .catch(() => {
      res.status(500).send("Error !! Cannot get the course !!");
    });
});
...
router.post("/", async (req, res) => {
  // validate the outputs before making a new course
...
router.patch("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the instructor of this course or sys admin can update this course.",
    });
  }
});
...
router.delete("/:_id", async (req, res) => {
  ...
  if (course.instructor.equals(req.user._id) || req.user.isAdmin()) {
    ...
    return res.json({
      success: false,
      message:
        "Only the instructor of this course or sys admin can delete this course.",
    });
  }
});
